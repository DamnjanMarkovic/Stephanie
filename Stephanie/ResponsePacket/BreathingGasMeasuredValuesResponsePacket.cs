...
namespace Stephanie
{
    ...
    internal class BreathingGasMeasuredValuesResponsePacket : ResponsePacket
    {
        #region Constants

        protected const byte SETTING_MESSAGE_ID = 0x36;

        #endregion

        #region Constructors

        public BreathingGasMeasuredValuesResponsePacket(byte[] byData)
            : base(byData)
        { }
...
#if DEBUG
            Debug.WriteLine($"BreathingGasMeasuredValuesResponsePacket; Data as byte array: {TestHelper.PrintByteArray(Data)}");
            Debug.WriteLine($"BreathingGasMeasuredValuesResponsePacket; Data as string: {Encoding.UTF8.GetString(Data)}");
#endif
